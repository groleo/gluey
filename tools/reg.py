...
class OutputGenerator:
    ...
    def beginFile(self, genOpts):
        ...
        if os.path.exists(self.genOpts.filename):
            write("File exists: %s" % self.genOpts.filename, file=sys.stderr)
        dname = os.path.dirname(self.genOpts.filename)
    ...
    def makeCDecls(self, cmd):
        ...
        for elem in proto:
            ...
            if (elem.tag == 'name'):
                ...
                tdecl += '(' + self.genOpts.apientryp + 'PFN' + text + 'PROC' + tail + ')'
            else:
                pdecl += text + tail
                tdecl += text + tail
